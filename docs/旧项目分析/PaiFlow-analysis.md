...
| **项目类型** | Java + TypeScript 双层架构 |
| **文件规模** | Java: 104+ 文件，TypeScript: 300+ 文件（Python 后端不在分析范围内） |
...
> **注意**：本分析仅关注 Java 后端（console/backend + core-workflow-java），Python 后端（core/）不在分析范围内。
...
- **功能**：执行工作流中的各个节点
- **关键类**：`AbstractNodeExecutor.java`
- **特点**：模板方法 + 策略模式，易于扩展新节点类型

### 4.3 Agent 调度模块

- **功能**：调度多个 Agent 协同工作
- **关键类**：`AgentManager.java`
...
1. **Java 引擎未完成**：core-workflow-java/ 是开发版，功能不完善
2. **SSE 连接管理**：缺乏连接心跳机制，可能导致死连接

### 8.2 架构问题

1. **插件体系复杂**：MCP/Link/自定义插件，集成成本高
2. **前后端耦合**：console/frontend 和 console/backend 耦合较紧

### 8.3 改进建议（对 zong 平台）

1. **完善 Java 工作流引擎**：补充 core-workflow-java/ 中缺失的功能
2. **完善 SSE 连接管理**：添加心跳机制、自动重连等
3. **简化插件体系**：统一插件接口，降低集成成本
...
1. **Thymeleaf 模板**：zong 使用 React，需完全重写
